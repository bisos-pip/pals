...
####+END:
    ...
    pathList = pathlib.Path(there).parts
    if len(pathList) < 5:
        icm.EH_problem_usageError(f"bad input: {there}")
        raise ValueError
    if pathList[0] != "/":
        icm.EH_problem_usageError(f"bad input: {there}")
        raise ValueError
    ...
    if pathList[3] != "iso":
        ...
        raise ValueError
    bpoId = pathList[4]
    bpoHome = os.path.expanduser(f"~{bpoId}")
...
####+BEGIN: bx:icm:python:cmnd:classHead :cmndName "palsBaseGetThere" :parsMand "" :parsOpt "" :argsMin "1" :argsMax "1" :asFunc "" :interactiveP ""
"""
*  _[[elisp:(blee:menu-sel:outline:popupMenu)][±]]_ _[[elisp:(blee:menu-sel:navigation:popupMenu)][Ξ]]_ [[elisp:(bx:orgm:indirectBufOther)][|>]] *[[elisp:(blee:ppmm:org-mode-toggle)][|N]]*  ICM-Cmnd   :: /palsBaseGetThere/ parsMand= parsOpt= argsMin=1 argsMax=1 asFunc= interactive=  [[elisp:(org-cycle)][| ]]
"""
class palsBaseGetThere(icm.Cmnd):
    cmndParamsMandatory = [ ]
    cmndParamsOptional = [ ]
...
####+END:
        ...
        cmndArgs = list(self.cmndArgsGet("1", cmndArgsSpecDict, effectiveArgsList)) # type: ignore

        print(cmndArgs)

        palsBaseThere(cmndArgs[0])
